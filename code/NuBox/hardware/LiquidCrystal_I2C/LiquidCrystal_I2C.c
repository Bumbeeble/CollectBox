...
//Library version:1.1
uint8_t Err_LCDI2C = 0;
#define _cols_lcdi2c_set	16
#define _rows_lcdi2c_set	2
LiquidCrystal_I2C_Def lcdi2c;
struct lcdi2c_buf_def{

	uint8_t buf[_rows_lcdi2c_set][_cols_lcdi2c_set];
	uint8_t diff[_rows_lcdi2c_set][_cols_lcdi2c_set];
};
...
/*
//Check LCDI2C Error and repair
void LCDI2C_CheckErr(uint8_t lcd_Addr,uint8_t lcd_cols,uint8_t lcd_rows)
{
	if(Err_LCDI2C)
	{
		Err_LCDI2C = 0;		//Clear Err 
	  ...
	  LCDI2C_begin(lcd_cols, lcd_rows);
		LCDI2C_noBacklight();
		LCDI2C_write_String("LCD Restart...");
		LCDI2C_backlight();
	}
}*/
...
/*	I2C_StartTransmission(lcdi2c.Addr,0);
	I2C_WriteData((_data) | lcdi2c.backlightval);
	I2C_Stop();
*/
	I2C_Start();
	I2C_SendByte(lcdi2c.Addr);//send the address for write
	I2C_WaitAck();
	I2C_SendByte((int)(_data) | lcdi2c.backlightval);
	I2C_WaitAck(); 
	I2C_Stop();

	/*
	I2C_StartTransmission (I2C1, I2C_Direction_Transmitter, lcdi2c.Addr, &Err_I2C); //Wire.beginTransmission(_Addr);
	Err_LCDI2C = Err_I2C;
	I2C_WriteData(I2C1, (int)(_data) | lcdi2c.backlightval, &Err_I2C);  //printIIC((int)(_data) | _backlightval);
	Err_LCDI2C = Err_I2C;
